...
struct ring_buffer* ring_buffer_init(void *buffer, uint32_t size)
{
    ...
    ring_buf->in = 0;
    ring_buf->out = 0;
	ring_buf->f_lock = malloc(sizeof(pthread_mutex_t));
	 if (!ring_buf->f_lock)
    {
        fprintf(stderr,"Failed to malloc memory,errno:%u,reason:%s",
            errno, strerror(errno));
	...
	pthread_mutex_init(ring_buf->f_lock,NULL);
...
//  @return free space data buffer len.
//缓冲区的长度
...
uint32_t __ring_buffer_put(struct ring_buffer *ring_buf, void *buffer, uint32_t size)
{
    assert(ring_buf || buffer);
    uint32_t len = 0;
    // find the smaller between  ring_buffer_len and request_data_len(size)
    size = min(size,__ring_buffer_len(ring_buf));
